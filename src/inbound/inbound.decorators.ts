...
/** @Inbound({ transport, ...spec }) — forma genérica (spec §7.2). */
...
/** REST: SetMetadata + UseInterceptors(InboundInterceptor) (spec §7.2). */
...
/** gRPC: SetMetadata + interceptor (spec §7.2). */
...
/** GraphQL (extensión ADR-015): mismo pipeline; operation informativa. */
...
/** Rabbit: SOLO metadata — el binding real vive en InboundExplorer (spec §7.2). */
