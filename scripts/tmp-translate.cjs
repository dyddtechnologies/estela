...
function tr(comment) {
  ...
  do {
    ...
    out = out.replace(RE, (m) => {
      const en = M[m.toLowerCase()];
      return en ? (/[A-Z]/.test(m[0]) ? en[0].toUpperCase() + en.slice(1) : en) : m;
    });
  } while (out !== prev);
  out = out.replace(
    /[^\x00-\x7F]/g,
...
function* walk(dir) {
  for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    else if (/\.(ts|js|mjs|cjs)$/.test(e.name)) yield p;
  }
}
  ...
  for (const file of walk(root)) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inBlock) {
        ...
        if (!inBlock && /\/\*/.test(t) && !/^\s*\//.test(t) === false && /^\s*\/\*/.test(t)) {
          inBlock = true;
        } else if (!inBlock && /\/\*/.test(t) && !/^\s*(\/\/)/.test(t) && !/^\s*\*/.test(t)) {
          comment = t;
          inBlock = true;
        }
